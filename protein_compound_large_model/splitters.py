...
def scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
                   ...
                   return_smiles=False):################dataset是分子图格式，smilies_list是smiles格式，分割按照骨架智能是按照smiles分，所以要引入smiles_list,最后将二者关联起来
    """
...
def scaffold_split_1(dataset, smiles_list, task_idx=None, null_value=0,
                   frac_train=0.8, frac_valid=0.1, frac_test=0.1,
                   return_smiles=False):################dataset是分子图格式，smilies_list是smiles格式，分割按照骨架智能是按照smiles分，所以要引入smiles_list,最后将二者关联起来
    
    ################只改了scaffold_split,未改random_split,scaffold_random_split，可以根据scaffold_split改
    """
    Adapted from https://github.com/deepchem/deepchem/blob/master/deepchem/splits/splitters.py
    Split dataset by Bemis-Murcko scaffolds
    This function can also ignore examples containing null values for a
    ...
    if task_idx != None:
        # filter based on null values in task_idx
        # get task array
        y_task = np.array([data.y[task_idx].item() for data in dataset.dataset2])
        # boolean array that correspond to non null values
        non_null = y_task != null_value
        smiles_list = list(compress(enumerate(smiles_list), non_null))#################
    else:
    ...
    mol_train_dataset = dataset.dataset2[torch.tensor(train_idx)]#$#################加torch.tensor,molecular is tensor
    mol_valid_dataset = dataset.dataset2[torch.tensor(valid_idx)]
    mol_test_dataset = dataset.dataset2[torch.tensor(test_idx)]
    
    seq_train_dataset=dataset.dataset1[(train_idx)]###############3去掉torch.tensor, sequence is not tensor
    seq_valid_dataset = dataset.dataset1[(valid_idx)]
    seq_test_dataset = dataset.dataset1[(test_idx)]
    print('seq_train_dataset:',type(seq_train_dataset))
  
    seq_train_dataset=SeqDataset(seq_train_dataset)
    seq_valid_dataset=SeqDataset(seq_valid_dataset)
    seq_test_dataset=SeqDataset(seq_test_dataset)
    
    smile_train_dataset=dataset.dataset3[(train_idx)]
    smile_valid_dataset = dataset.dataset3[(valid_idx)]
    smile_test_dataset = dataset.dataset3[(test_idx)]
    print('seq_train_dataset:',type(seq_train_dataset))
  
    smile_train_dataset=SmileDataset(smile_train_dataset)
    smile_valid_dataset=SmileDataset(smile_valid_dataset)
    smile_test_dataset=SmileDataset(smile_test_dataset)
    
    if not return_smiles:
        return mol_train_dataset, mol_valid_dataset, mol_test_dataset,seq_train_dataset,seq_valid_dataset,seq_test_dataset,smile_train_dataset,smile_valid_dataset,smile_test_dataset
    else:
        train_smiles = [smiles_list[i][1] for i in train_idx]
        valid_smiles = [smiles_list[i][1] for i in valid_idx]
        test_smiles = [smiles_list[i][1] for i in test_idx]
