...
class CrossAttentionLayer1(nn.Module):
    def __init__(self, input_dim1, input_dim2):
        super(CrossAttentionLayer, self).__init__()
        self.linear1 = nn.Linear(input_dim1, input_dim2)###################妙
        self.linear2 = nn.Linear(input_dim2, input_dim1)
...
class InteractionModel_4(torch.nn.Module):
    ...
    def forward(self, protein_data,molecular_data,smile_data):#
        ...
        with torch.no_grad():
            #results = self.protein_model(batch_tokens, repr_layers=[33], return_contacts=True)###############3
            results = self.protein_model(batch_tokens, repr_layers=[12], return_contacts=False)###############3
        protein_token_representations = results["representations"][12]
        del results
        m,n,v=protein_token_representations.shape
        #print('m,n,v:',m,n,v)
        #print('protein_token_representations:',protein_token_representations.shape)
        ...
        ################molecular
        del portein_token_representations,u
        ...
        ###x11_att=x1*fused_x2##=================================
        ###x22_att=x2*fused_x1####==========================
        x11_att=fused_x2########drop off multlipy,and add directly
        x22_att=fused_x1
        ...
        ###x33_att=x3*fused_x4##################drop off multipy and add directly
        ####x44_att=x4*fused_x3
