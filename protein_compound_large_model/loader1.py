...
class TestbedDataset(Dataset):
    ...
    def __init__(self, root='/tmp', dataset='davis',test=False, 
                 transform=None,
                 pre_transform=None):
    ...
    def raw_file_names(self):
        #pass
        file_name_list = os.listdir(self.raw_dir)###########3very important
        # assert len(file_name_list) == 1     # currently assume we have a
        # # single raw file
        return file_name_list
    ...
    def processed_file_names(self):
        ...
        if self.test:
            return [self.dataset+f'_test_mols.pt' ]
        else:
            return [self.dataset+f'_train_mols.pt']
    ...
    def process(self):
        ...
        for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:  # ignore invalid mol objects
                    ...
                    my_data = mol_to_graph_data_obj_simple(rdkit_mol)###################转化成了x.data,x.index_edge........
                    # manually add mol id
                    my_data.id = torch.tensor([i])
                    my_data.y = torch.tensor([labels[i]])
        ...
        if self.dataset=='davis':
            ...
            if self.test:
                torch.save(data_list, os.path.join(self.processed_dir, 'davis_test_mols.pt'))

                # Save preprocessed data
                data_smiles_series = pd.Series(data_smiles_list)
                data_smiles_series.to_csv(os.path.join(self.processed_dir, 'davis_test_smiles.csv'), index=False, header=False)

                data_sequence_series = pd.Series(data_sequence_list)
                data_sequence_series.to_csv(os.path.join(self.processed_dir, 'davis_test_sequence.csv'), index=False, header=None)
                # Save data_list as a .pt file
            else:
                torch.save(data_list, os.path.join(self.processed_dir, 'davis_train_mols.pt'))

                # Save preprocessed data
                data_smiles_series = pd.Series(data_smiles_list)
                data_smiles_series.to_csv(os.path.join(self.processed_dir, 'davis_train_smiles.csv'), index=False, header=False)

                data_sequence_series = pd.Series(data_sequence_list)
                data_sequence_series.to_csv(os.path.join(self.processed_dir, 'davis_train_sequence.csv'), index=False, header=None)
        if self.dataset=='kiba':
            if self.test:
                # Save data_list as a .pt file
                torch.save(data_list, os.path.join(self.processed_dir, 'kiba_test_mols.pt'))

                # Save preprocessed data
                data_smiles_series = pd.Series(data_smiles_list)
                data_smiles_series.to_csv(os.path.join(self.processed_dir, 'kiba_test_smiles.csv'), index=False, header=False)

                data_sequence_series = pd.Series(data_sequence_list)
                data_sequence_series.to_csv(os.path.join(self.processed_dir, 'kiba_test_sequence.csv'), index=False, header=None)
            else:
                # Save data_list as a .pt file
                torch.save(data_list, os.path.join(self.processed_dir, 'kiba_train_mols.pt'))

                # Save preprocessed data
                data_smiles_series = pd.Series(data_smiles_list)
                data_smiles_series.to_csv(os.path.join(self.processed_dir, 'kiba_train_smiles.csv'), index=False, header=False)

                data_sequence_series = pd.Series(data_sequence_list)
                data_sequence_series.to_csv(os.path.join(self.processed_dir, 'kiba_train_sequence.csv'), index=False, header=None)
...
class myMoleculeDataset(Dataset):
    def __init__(self, root, test=False,transform=None, pre_transform=None):
        ...
        #self.raw_file = self.root+'/raw/*.csv'
    ...
    def process(self):
        ...
        for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:  # ignore invalid mol objects
                    ...
                    # 保存图数据
                    if self.test:
                        #torch.save(self.collate(data_list), os.path.join(self.processed_dir, f'data_test_{i}.pt'))
                        torch.save(data_list, os.path.join(self.processed_dir, f'data_test_{i}.pt'))
                    else:
                        #torch.save(self.collate(data_list), os.path.join(self.processed_dir, f'data_{i}.pt'))
                        torch.save(data_list, os.path.join(self.processed_dir, f'data_{i}.pt'))
                    '''
        
        
        if self.pre_filter is not None:
            data_list = [self.pre_filter(mdata) for mdata in data_list]

        if self.pre_transform is not None:
            data_list = [self.pre_transform(mdata) for mdata in data_list]
    ...
    def get(self, idx):
        #my_data = torch.load(self.processed_paths[idx])
        #return my_data
        if self.test:
            data = torch.load(os.path.join(self.processed_dir, f'data_test_{idx}.pt'))
        else:
            data = torch.load(os.path.join(self.processed_dir, f'data_{idx}.pt')) #####################    
        #print(os.path.join(self.processed_dir,f'data_{idx}.pt'))
        return data
...
class MoleculeDatasetBig(Dataset):#InMemoryDataset转换成分子图
    ...
    def __init__(self,
        ...
        self.img_path=root+'/processed/'
        print('self.img_path:',self.img_path)
        if os.path.exists(self.img_path):
            
            self.data_dir=pathlib.Path(self.img_path)
            self.dataset1=list(self.data_dir.glob('data_*.pt'))
        
            self.data=self.dataset1
            print('self.dataset1:',len(self.dataset1))
            
            for file in self.dataset1:
                #print('file:',file)
                torch.load(file)
