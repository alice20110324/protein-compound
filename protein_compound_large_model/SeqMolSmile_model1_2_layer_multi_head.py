...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        self.relu=nn.ReLU()
        self.fc_out1=nn.Linear(embed_size,256)
    ...
    def forward(self, values, keys, queries, mask=None):
        ...
        # Apply final linear layer and return
        out = self.relu(self.fc_out(out))
        out=self.dropout(self.relu(self.fc_out1(out)))
        return out
...
class InteractionModel_5(torch.nn.Module):
    def __init__(self, protein_model,molecular_model,smile_model,protein_embd_dim,mol_embd_dim,num_tasks, device,smile_embd_dim, dropout=0.2,aggr = "mean"):
        ...
        self.mol_cross_attention=MultiHeadAttention(1,1)
        #self.fc1 = nn.Linear(protein_embd_dim+ mol_embd_dim, 256)
        ...
        self.smile_cross_attention=MultiHeadAttention(1,1)
        #self.fc3 = nn.Linear(protein_embd_dim+ smile_embd_dim, 256)
        ...
        '''
        #self.fc_embd=torch.nn.Linear(256*6,256)
        self.sigmoid=nn.Sigmoid()
    ...
    def forward(self, protein_data,molecular_data,smile_data):#
        ...
        x22=  self.mol_cross_attention(x2,x2,x1)
        
        x111=  self.mol_cross_attention(x11,x11,x22) 
        x222=  self.mol_cross_attention(x22,x22,x11)
        ...
        #fused_x3, fused_x4 =  self.smile_cross_attention(x3, x4,self.protein_embd_dim,self.smile_embd_dim)  
        x33 =  self.smile_cross_attention(x3, x3,x4)  
        x44 =  self.smile_cross_attention(x4, x4,x43) 
        
        
        
        x333 =  self.smile_cross_attention(x33, x33,x44)  
        x444 =  self.smile_cross_attention(x44, x44,x33)
