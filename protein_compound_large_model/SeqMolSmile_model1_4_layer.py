...
class InteractionModel_5(torch.nn.Module):
    ...
    def forward(self, protein_data,molecular_data,smile_data):#
        ...
        fused_x1, fused_x2 =  self.mol_cross_attention(x111, x222,256,256) 
        x111_att=fused_x1
        x222_att=fused_x2
        
        x1111=torch.add(x111_att,x111)
        x2222=torch.add(x222_att,x222)
        
        fused_x1, fused_x2 =  self.mol_cross_attention(x1111, x2222,256,256) 
        x1111_att=fused_x1
        x2222_att=fused_x2
        
        x11111=torch.add(x1111_att,x1111)
        x22222=torch.add(x2222_att,x2222)
        ...
        del x1111_att, x2222_att
        del x111,x222
        
        del x11111_att, x22222_att
        del x1111,x2222
        ...
        #torch.cuda.empty_cache()
        # 合并两个融合后的向量
        combined12 = torch.cat([x11111_att, x22222], dim=1)   
