...
class InteractionModel_4(torch.nn.Module):
    def __init__(self, protein_model,molecular_model,smile_model,protein_embd_dim,mol_embd_dim,num_tasks, device,smile_embd_dim, dropout=0.2,aggr = "mean"):
        ...
        self.conv1=nn.Sequential(
            ...
            nn.BatchNorm1d(1580),
            nn.MaxPool1d(kernel_size=5,stride=5),#316
            self.dropout,
            #F.relu(),
        ...
        self.conv2=nn.Sequential(
            ...
            self.dropout,
            #F.relu(),
            nn.Conv1d(in_channels=64,out_channels=1,kernel_size=5,stride=1,padding=2),
            nn.BatchNorm1d(11),
            nn.MaxPool1d(kernel_size=5,stride=5),
            self.dropout
            #F.relu()
    ...
    def forward(self, protein_data,molecular_data,smile_data):#
        ...
        #print('fused_x1,fused_x2:',fused_x1.shape,fused_x2.shape)
        #fused_x1=F.softmax(fused_x1)
        #fused_x2=F.softmax(fused_x2)
        #print('x1:',x1.shape)
        #print('x2:',x2.shape)
        ...
        #print('combined12:',combined12.shape)
        #all_features=torch.concat([self.protein_sequence_representations,molecular_representation],axis=1)
        #print('all_features:',all_features.shape)
        combined12=combined12.reshape(-1,1,self.protein_embd_dim+self.mol_embd_dim)
        out12=self.conv1(combined12)
        #print('out12:',out12.shape)
        out12=self.conv2(out12)
        #print('out12:',out12.shape)
        out12=out12.squeeze(1)
        #print('out12:',out12.shape)
        linear12=nn.Linear(out12.shape[1],1).to(self.device)
        out12=linear12(out12)
        
        #out12 = self.fc1(combined12)
        #out12 = self.dropout(self.relu(out12))
        #out12 = torch.sigmoid(self.fc2(out12)) 
        ...
        #print('fused_x3,fused_x4:',fused_x3.shape,fused_x4.shape)
        ...
        #print('combined34:',combined34.shape)
        combined34=combined34.reshape(-1,1,self.protein_embd_dim+self.smile_embd_dim)
        #all_features=torch.concat([self.protein_sequence_representations,molecular_representation],axis=1)
        #print('all_features:',all_features.shape)
        out34=self.conv3(combined34)
        out34=self.conv4(out34)
        out34=out34.squeeze(1)
        #print('out34:',out34.shape)
        linear1=nn.Linear(out34.shape[1],1).to(self.device)
        out34=linear1(out34)
