...
class InteractionModel_4(torch.nn.Module):
    def __init__(self, protein_model,molecular_model,smile_model,protein_embd_dim,mol_embd_dim,num_tasks, device,smile_embd_dim, dropout=0.2,aggr = "mean"):
        ...
        self.fc1 = nn.Linear(256+ 256, 100)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(100, 1)
        ...
        self.fc3 = nn.Linear(256+ 256, 100)
    ...
    def forward(self, protein_data,molecular_data,smile_data):#
        ...
        for i,p in enumerate(self.protein_model.parameters()):
            p.requires_grad = False#freezing parameters
        for i,p in enumerate(self.smile_model.parameters()):
            p.requires_grad = False
        for i,p in enumerate(self.mole_model.parameters()):
            p.requires_grad = False
        #with torch.no_grad():
        ...
        for i, tokens_len in enumerate(batch_lens):
            if i==0:
                ...
                print('protein_sequence_representations:',self.protein_sequence_representations.shape)
        self.protein_sequence_representations=self.protein_linear(self.protein_sequence_representations)############fine_tunning protein large model 
        #self.protein_sequence_representations=self.protein_sequence_representations############
        #set up optimizer
        #different learning rate for different part of GNN
        #with torch.no_grad(): 
        molecular_node_representation=self.mole_model(molecular_data.x,molecular_data.edge_index,molecular_data.edge_attr)
        ...
        molecular_representation=self.mole_linear(molecular_representation)
        ...
        #print('normal_x2:',x2.shape)
        #print('normal_x3:',x3.shape)
        #x1=self.layer_norm_seq(x1)
        #x2=self.layer_norm_mol(x2)
        #x3=self.layer_norm_seq(x3)
        
        # 合并两个融合后的向量
        combined12 = torch.cat([x1, x2], dim=1) 
        ...
        out12=self.relu(self.fc2(out12))
        #s=self.sig
        #out12_=s(out12)
        #out=self.pred_linear(all_features)
        #print('out12:',out12)
